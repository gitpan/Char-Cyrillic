...
my $function_ord;         # ord()   to ord()   or Cyrillic::ord()
my $function_ord_;        # ord     to ord     or Cyrillic::ord_
my $function_reverse;     # reverse to reverse or Cyrillic::reverse
my $function_getc;        # getc    to getc    or Cyrillic::getc
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Cyrillic::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Cyrillic::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Cyrillic::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Cyrillic::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# functions of package Ecyrillic
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Cyrillic::re_t=%s; while($Cyrillic::re_t =~ %s){%s local $^W=0; local $Cyrillic::re_r=%s; %s$Cyrillic::re_t="$`$Cyrillic::re_r$'"; pos($Cyrillic::re_t)=length "$`$Cyrillic::re_r"; } return $Cyrillic::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Cyrillic::re_r=eval $Cyrillic::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Cyrillic::re_r=%s; %s"%s$Cyrillic::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Cyrillic::re_n=0; while(%s =~ %s){%s local $^W=0; local $Cyrillic::re_r=%s; %s%s="$`$Cyrillic::re_r$'"; pos(%s)=length "$`$Cyrillic::re_r"; $Cyrillic::re_n++} return %s$Cyrillic::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Cyrillic::re_r=%s; %s%s="%s$Cyrillic::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Cyrillic::re_r=%s; %s%s="%s$Cyrillic::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Cyrillic::re_r=eval $Cyrillic::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
expand wildcard arguments supplied to programs. Instead, import of Ecyrillic.pm
works well.
...
You need write 'use Cyrillic;' in your script.
  ...
  Before      You do
  ---------------------------------
  (nothing)   use Cyrillic;
  ---------------------------------

=head1 Calling 'Ecyrillic::ignorecase()' (Cyrillic.pm provides)

Cyrillic.pm applies calling 'Ecyrillic::ignorecase()' instead of /i modifier.
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Cyrillic.pm provides)

Cyrillic.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Cyrillic.pm and Ecyrillic.pm provide)
...
=head1 Un-Escaping bytes::* Functions (Cyrillic.pm provides)

Cyrillic.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Ecyrillic.pm provides)

Ecyrillic.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Cyrillic' }" at head.
Store the standard module modified for Cyrillic software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Cyrillic and change
'use utf8;' to 'use Cyrillic;' in its. You need help yourself for now.
