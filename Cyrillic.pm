...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Ecyrillic::uc($char[$i]) ne Ecyrillic::lc($char[$i]))) {
            $char[$i] = '[' . Ecyrillic::uc($char[$i]) . Ecyrillic::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Cyrillic/Cyrillic/JA.pm
