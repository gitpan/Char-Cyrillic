...
BEGIN {
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Ecyrillic::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Ecyrillic::open;
    }
}
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Ecyrillic::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Ecyrillic::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Ecyrillic::split returns
...
=item Chop string
  ...
  The Ecyrillic::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Ecyrillic::index($string,$substr,$position);
  $pos = Ecyrillic::index($string,$substr);
  ...
  the Ecyrillic::index function returns -1.

=item Reverse index string

  $pos = Ecyrillic::rindex($string,$substr,$position);
  $pos = Ecyrillic::rindex($string,$substr);

  Works just like Ecyrillic::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Ecyrillic::chr(65) is "A" in either ASCII or Cyrillic, and
  Ecyrillic::chr(0x82a0) is a Cyrillic HIRAGANA LETTER A. For the reverse of Ecyrillic::chr,
  use Cyrillic::ord.
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Ecyrillic::binmode(FILEHANDLE, $disciplines);
  Ecyrillic::binmode(FILEHANDLE);
  Ecyrillic::binmode($filehandle, $disciplines);
  Ecyrillic::binmode($filehandle);

  * two arguments

  If you are using perl5.005 other than MacPerl, Cyrillic software emulate perl5.6's
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Ecyrillic::open(FILEHANDLE, $mode, $expr);
  $rc = Ecyrillic::open(FILEHANDLE, $expr);
  $rc = Ecyrillic::open(FILEHANDLE);
  $rc = Ecyrillic::open(my $filehandle, $mode, $expr);
  $rc = Ecyrillic::open(my $filehandle, $expr);
  $rc = Ecyrillic::open(my $filehandle);

  * autovivification filehandle
  * three arguments

  If you are using perl5.005, Cyrillic software emulate perl5.6's open function.
  ...
  {
      my $fh;                          # (uninitialized)
      Ecyrillic::open($fh, ">logfile")     # $fh is autovivified
  ...
  Ecyrillic::open my $fh, ">logfile" or die ...
  ...
  Ecyrillic::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Ecyrillic::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Ecyrillic::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Ecyrillic::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Ecyrillic::open(FH, "<:crlf:encoding(Cyrillic)", $path) or die "can't open $path: $!";
