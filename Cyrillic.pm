...
######################################################################
#
# Cyrillic - Source code filter to Cyrillic script
#
...
sub Cyrillic::escape_script {
    ...
    if (/^ use Ecyrillic(?:\s+[0-9\.]*)?\s*; $/oxms) {
        ...
        $e_script .= sprintf("use Ecyrillic %s;\n%s", $Ecyrillic::VERSION, $use_re_eval); # require run-time routines version

        # use Cyrillic version qw(ord reverse);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
...
Cyrillic - Source code filter to escape Cyrillic

=head1 SYNOPSIS

  use Cyrillic;
  use Cyrillic version;         --- require version
...
Definitions in Ecyrillic.pm.
  ...
  @{Ecyrillic::not_space}      qr{(?:[^\x09\x0A\x0B\x0C\x0D\x20])}
...
Contrasting is Encode module and describing "use Cyrillic;" on this software,
