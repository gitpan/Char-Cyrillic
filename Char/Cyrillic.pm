...
# split
            ...
            my $e_string = 'Char::Ecyrillic::split';
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and (Char::Ecyrillic::uc($char[$i]) ne Char::Ecyrillic::lc($char[$i]))) {
...
and the cellular phone. This software treats Cyrillic directly. Therefor there is
not UTF8 flag.
...
=item * Possible to handle raw Cyrillic values

=item * Backward compatibility of data, script and how to

=item * Only Char/Cyrillic.pm and Char/Ecyrillic.pm, other modules are unnecessary
...
functions.
  ...
  Before      After
  ---------------------------------
  lc          Char::Ecyrillic::lc
  lcfirst     Char::Ecyrillic::lcfirst
  uc          Char::Ecyrillic::uc
  ucfirst     Char::Ecyrillic::ucfirst
  chr         Char::Ecyrillic::chr
  glob        Char::Ecyrillic::glob
  ---------------------------------
...
Char/Cyrillic.pm remove 'bytes::' at head of function name.
...
=head1 Perl5.6 Emulation on perl5.005
  ...
  --------------------------------------------------------------------
  Before          After                  in BEGIN { } of Char/Ecyrillic.pm
  --------------------------------------------------------------------
  binmode(...);   Char::Ecyrillic::binmode(...);   *CORE::GLOBAL::binmode = ...
  open(...);      Char::Ecyrillic::open(...);      *CORE::GLOBAL::open    = ...
  --------------------------------------------------------------------
...
=item * binmode (Perl5.6 emulation on perl5.005)
  ...
  * two arguments
  ...
  If you are using perl5.005, Char::Cyrillic software emulate perl5.6's binmode function.
...
=item * open (Perl5.6 emulation on perl5.005)
  ...
  * three arguments
  ...
  open(FH, "<:crlf:encoding(Char::Cyrillic)", $path) or die "can't open $path: $!";
...
=head1 Ignore utf8 pragma
  ...
  ---------------------------------------------------------------------
  use utf8;       # use utf8;            Char/Ecyrillic.pm provides utf8::*
  no utf8;        # no utf8;             functions even if 'no utf8;'
  use bytes;      # use bytes;           Char/Ecyrillic.pm provides bytes::*
...
page 402 of Programming Perl, 3rd ed. again.
  ...
  Information processing model beginning with this software
    ...
    +-----------------------------------+
    |      ASCII Compatible Encoding    | ex. Cyrillic
    +-----------------------------------+
                (No UTF8 Flag)
...
I am thankful to all persons.
 ...
 http://search.cpan.org/dist/Cyrillic-Regexp/
