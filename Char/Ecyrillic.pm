...
BEGIN {
    ...
    sub Char::Ecyrillic::binmode(*;$);
    sub Char::Ecyrillic::open(*;$@);
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Char::Ecyrillic::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Char::Ecyrillic::open;
    }
}
...
=item Make character
  ...
  set. For example, Char::Ecyrillic::chr(65) is "A" in either ASCII or Cyrillic, and
  Char::Ecyrillic::chr(0x82a0) is a Cyrillic HIRAGANA LETTER A. For the reverse of Char::Ecyrillic::chr,
  use Char::Cyrillic::ord.
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Char::Ecyrillic::binmode(FILEHANDLE, $disciplines);
  Char::Ecyrillic::binmode(FILEHANDLE);
  Char::Ecyrillic::binmode($filehandle, $disciplines);
  Char::Ecyrillic::binmode($filehandle);

  * two arguments

  If you are using perl5.005 other than MacPerl, Char::Cyrillic software emulate perl5.6's
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Char::Ecyrillic::open(FILEHANDLE, $mode, $expr);
  $rc = Char::Ecyrillic::open(FILEHANDLE, $expr);
  $rc = Char::Ecyrillic::open(FILEHANDLE);
  $rc = Char::Ecyrillic::open(my $filehandle, $mode, $expr);
  $rc = Char::Ecyrillic::open(my $filehandle, $expr);
  $rc = Char::Ecyrillic::open(my $filehandle);

  * autovivification filehandle
  * three arguments

  If you are using perl5.005, Char::Cyrillic software emulate perl5.6's open function.
  ...
  {
      my $fh;                          # (uninitialized)
      Char::Ecyrillic::open($fh, ">logfile")     # $fh is autovivified
  ...
  Char::Ecyrillic::open my $fh, ">logfile" or die ...
  ...
  Char::Ecyrillic::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Char::Ecyrillic::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Char::Ecyrillic::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Char::Ecyrillic::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Char::Ecyrillic::open(FH, "<:crlf:encoding(Char::Cyrillic)", $path) or die "can't open $path: $!";
