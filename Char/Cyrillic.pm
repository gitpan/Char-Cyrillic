...
######################################################################
#
# Char::Cyrillic - Source code filter to escape Cyrillic script
#
# http://search.cpan.org/dist/Char-Char::Cyrillic/
#
...
BEGIN { CORE::require Char::Ecyrillic; }
...
sub unimport {}
sub Char::Cyrillic::escape_script;
...
my $function_ord;         # ord()   to ord()   or Char::Cyrillic::ord()
my $function_ord_;        # ord     to ord     or Char::Cyrillic::ord_
my $function_reverse;     # reverse to reverse or Char::Cyrillic::reverse
my $function_getc;        # getc    to getc    or Char::Cyrillic::getc
...
END
    ...
    print Char::Cyrillic::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Char::Ecyrillic::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::Cyrillic::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Char::Ecyrillic::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::Cyrillic::escape_script {
    ...
    my $fh = gensym();
    Char::Ecyrillic::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Ecyrillic(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Ecyrillic %s;\n", $Char::Cyrillic::VERSION); # require run-time routines version

        # use Char::Cyrillic version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::Cyrillic \s* ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::Cyrillic::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/Cyrillic.pm}} @INC;
...
****************************************************
              ...
              CONFLICT Char/Cyrillic.pm FILE
...
END
                    ...
                    die "Script $0 expects Char/Cyrillic.pm $version, but @{[__FILE__]} is version $Char::Cyrillic::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Ecyrillic.pm $version, but \\\$Char::Ecyrillic::VERSION is \$Char::Ecyrillic::VERSION" if \$Char::Ecyrillic::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::Cyrillic::VERSION) {
                    die "Script $0 required Char/Cyrillic.pm $version, but @{[__FILE__]} is only version $Char::Cyrillic::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::Cyrillic::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::Cyrillic::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::Cyrillic::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::Cyrillic::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# escape Cyrillic part of script
sub escape {
...
# functions of package Char::Ecyrillic
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::Cyrillic::re_t=%s; while($Char::Cyrillic::re_t =~ %s){%s local $^W=0; local $Char::Cyrillic::re_r=%s; %s$Char::Cyrillic::re_t="$`$Char::Cyrillic::re_r$'"; pos($Char::Cyrillic::re_t)=length "$`$Char::Cyrillic::re_r"; } return $Char::Cyrillic::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::Cyrillic::re_r=eval $Char::Cyrillic::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::Cyrillic::re_r=%s; %s"%s$Char::Cyrillic::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::Cyrillic::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::Cyrillic::re_r=%s; %s%s="$`$Char::Cyrillic::re_r$'"; pos(%s)=length "$`$Char::Cyrillic::re_r"; $Char::Cyrillic::re_n++} return %s$Char::Cyrillic::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::Cyrillic::re_r=%s; %s%s="%s$Char::Cyrillic::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::Cyrillic::re_r=%s; %s%s="%s$Char::Cyrillic::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::Cyrillic::re_r=eval $Char::Cyrillic::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/Cyrillic.pm and Char/Ecyrillic.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::Cyrillic;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::Cyrillic;
  use Char::Cyrillic ver.sion;             --- require minimum version
  use Char::Cyrillic ver.sion.0;           --- expects version (match or die)

  # "no Char::Cyrillic;" not supported

  or

  $ perl Char/Cyrillic.pm Cyrillic_script.pl > Escaped_script.pl.e
...
Char::Cyrillic software is "middleware" between perl interpreter and your Perl script
written in Cyrillic.
...
By "use Char::Cyrillic;", it automatically interpret your script as Cyrillic. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/Cyrillic.pm, Char/Ecyrillic.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Ecyrillic.pm
works well.

   in Char/Ecyrillic.pm
   #
...
=head1 Software Composition
   ...
   Char/Cyrillic.pm               --- source code filter to escape Cyrillic
   Char/Ecyrillic.pm              --- run-time routines for Char/Cyrillic.pm
...
You need write 'use Char::Cyrillic;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::Cyrillic;
  ---------------------------------

=head1 Calling 'Char::Ecyrillic::ignorecase()' (Char/Cyrillic.pm provides)

Char/Cyrillic.pm applies calling 'Char::Ecyrillic::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Ecyrillic::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Ecyrillic.pm provides)
...
The character classes are redefined as follows to backward compatibility.
  ...
  ---------------------------------------------------------------
   .            ${Char::Ecyrillic::dot}
                ${Char::Ecyrillic::dot_s}    (/s modifier)
  \d            [0-9]
  \s            \s
  \w            [0-9A-Z_a-z]
  \D            ${Char::Ecyrillic::eD}
...
Definitions in Char/Ecyrillic.pm.
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/Cyrillic.pm provides)

Char/Cyrillic.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/Cyrillic.pm and Char/Ecyrillic.pm provide)

Insert 'Char::Ecyrillic::' at head of function name. Char/Ecyrillic.pm provides your script Char::Ecyrillic::*
functions.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Char::Ecyrillic::lc        Character
  lcfirst     Char::Ecyrillic::lcfirst   Character
  uc          Char::Ecyrillic::uc        Character
  ucfirst     Char::Ecyrillic::ucfirst   Character
  fc          Char::Ecyrillic::fc        Character
  chr         Char::Ecyrillic::chr       Character
  glob        Char::Ecyrillic::glob      Character
  -------------------------------------------
...
=head1 Un-Escaping bytes::* Functions (Char/Cyrillic.pm provides)

Char/Cyrillic.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Char/Ecyrillic.pm provides)

Char/Ecyrillic.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Char::Cyrillic' }" at head.
Store the standard module modified for Char::Cyrillic software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Char::Cyrillic and change
'use utf8;' to 'use Char::Cyrillic;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Ecyrillic.pm provides these
  functions.
...
Char::Cyrillic software remains one language and one interpreter.
