...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Ecyrillic::uc($char[$i]) ne Char::Ecyrillic::lc($char[$i]))) {
            $char[$i] = '[' . Char::Ecyrillic::uc($char[$i]) . Char::Ecyrillic::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Char::Cyrillic/Char::Cyrillic/JA.pm
...
Definitions in Char/Ecyrillic.pm.
  ...
  @{Char::Ecyrillic::eV}             qr{(?:[^\x0C\x0A\x0D])}
