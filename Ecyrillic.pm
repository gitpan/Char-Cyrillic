...
######################################################################
#
# Ecyrillic - Run-time routines for Cyrillic.pm
#
...
BEGIN {
    ...
    sub qualify ($;$) {
        my ($name) = @_;
        if (!ref($name) && (Ecyrillic::index($name, '::') == -1) && (Ecyrillic::index($name, "'") == -1)) {
            my $pkg;
...
sub Ecyrillic::chop(@);
sub Ecyrillic::index($$;$);
sub Ecyrillic::rindex($$;$);
sub Ecyrillic::lcfirst(@);
sub Ecyrillic::lcfirst_();
sub Ecyrillic::lc(@);
sub Ecyrillic::lc_();
sub Ecyrillic::ucfirst(@);
sub Ecyrillic::ucfirst_();
sub Ecyrillic::uc(@);
sub Ecyrillic::uc_();
sub Ecyrillic::fc(@);
sub Ecyrillic::fc_();
sub Ecyrillic::ignorecase;
sub Ecyrillic::classic_character_class;
sub Ecyrillic::capture;
sub Ecyrillic::chr(;$);
sub Ecyrillic::chr_();
sub Ecyrillic::glob($);
sub Ecyrillic::glob_();
...
# ${Ecyrillic::eS}        = qr{(?:[^\x09\x0A    \x0C\x0D\x20])};
# ${Ecyrillic::eS}        = qr{(?:[^\x09\x0A\x0B\x0C\x0D\x20])};
${Ecyrillic::eS}          = qr{(?:[^\s])};
...
${Ecyrillic::not_lower_i} = qr{(?:[\x00-\xFF])};
...
${Ecyrillic::not_upper_i} = qr{(?:[\x00-\xFF])};
...
sub Ecyrillic::lcfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Ecyrillic::lc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Ecyrillic::lc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Ecyrillic::lc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}
...
sub Ecyrillic::ucfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Ecyrillic::uc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Ecyrillic::uc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Ecyrillic::uc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}
    ...
    for my $string (@string) {
        ...
        for (my $i=0; $i <= $#char; $i++) {
            ...
            # with /i modifier
                ...
                my $uc = Ecyrillic::uc($char[$i]);
                my $fc = Ecyrillic::fc($char[$i]);
                if ($uc ne $fc) {
                    if (CORE::length($fc) == 1) {
                        $char[$i] = '['   . $uc       . $fc . ']';
                    }
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # POSIX-style character classes
            ...
            $char[$i] = {
                ...
                '[:^lower:]'  => '${Ecyrillic::not_lower_i}',
                '[:^upper:]'  => '${Ecyrillic::not_upper_i}',
            ...
            for (@singleoctet) {
                while (s/ \A \\x(..) - \\x(..) //oxms or s/ \A \\x((..)) //oxms) {
                    for my $ord (hex($1) .. hex($2)) {
                        ...
                        my $uc = Ecyrillic::uc($char);
                        my $fc = Ecyrillic::fc($char);
                        if ($uc eq $fc) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $fc = Ecyrillic::fc($char)) ne $char) {
                $pattern .= $fc;
            }
            else {
        ...
        my $matchsub = sub { Ecyrillic::fc($_[0]) =~ /\A $pattern \z/xms };
...
INNER:
        for my $leaf (@leaf) {
            ...
            if (Ecyrillic::index($leaf,'.') == -1 and   # if name does not have a dot in it *and*
                CORE::length($leaf) <= 8 and        # name is shorter than or equal to 8 chars *and*
                Ecyrillic::index($pattern,'\\.') != -1  # pattern has a dot.
            ) {
                if (&$matchsub("$leaf.")) {
                    push @matched, "$head$leaf";
                    next INNER;
...
sub Cyrillic::getc(;*@) {
    ...
    croak 'Too many arguments for Cyrillic::getc' if @_ and not wantarray;
    ...
    for my $length ($length[0] .. $length[-1]) {
        ...
        if (exists $range_tr{CORE::length($getc)}) {
            if ($getc =~ /\A ${Ecyrillic::dot_s} \z/oxms) {
...
sub Cyrillic::index($$;$) {
    ...
    my $index;
    if (@_ == 3) {
        $index = Ecyrillic::index($_[0], $_[1], CORE::length(Cyrillic::substr($_[0], 0, $_[2])));
    }
    else {
        $index = Ecyrillic::index($_[0], $_[1]);
    }
    ...
    if ($index == -1) {
        ...
        return Cyrillic::length(CORE::substr $_[0], 0, $index);
    }
}
...
sub Cyrillic::rindex($$;$) {
    ...
    my $rindex;
    if (@_ == 3) {
        $rindex = Ecyrillic::rindex($_[0], $_[1], CORE::length(Cyrillic::substr($_[0], 0, $_[2])));
    }
    else {
        $rindex = Ecyrillic::rindex($_[0], $_[1]);
    }
    ...
    if ($rindex == -1) {
        ...
        return Cyrillic::length(CORE::substr $_[0], 0, $rindex);
    }
}
...
my $qq_substr  = qr{(?: Cyrillic::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
sub Cyrillic::escape(;$) {
    ...
    while (not /\G \z/oxgc) { # member
        $e_script .= Cyrillic::escape_token();
    }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Ecyrillic::PREMATCH()
    elsif (/\G ( \$` | \$\{`\} | \$ \s* PREMATCH  \b | \$ \s* \{ \s* PREMATCH  \s* \} | \$ \s* \{\^PREMATCH\}  ) /oxmsgc) {
        $slash = 'div';
        return q{Ecyrillic::PREMATCH()};
    }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Ecyrillic::MATCH()
    elsif (/\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
        $slash = 'div';
        return q{Ecyrillic::MATCH()};
    }
...
# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Ecyrillic::POSTMATCH()
    elsif (/\G (                 \$ \s* POSTMATCH \b | \$ \s* \{ \s* POSTMATCH \s* \} | \$ \s* \{\^POSTMATCH\} ) /oxmsgc) {
        $slash = 'div';
        return q{Ecyrillic::POSTMATCH()};
    }
...
# while (<WILDCARD>) --- glob
    ...
    # avoid "Error: Runtime exception" of perl version 5.005_03
        ...
        return 'while ($_ = Ecyrillic::glob("' . $1 . '"))';
    }

# while (glob)
    elsif (/\G \b while \s* \( \s* glob \s* \) /oxgc) {
        return 'while ($_ = Ecyrillic::glob_)';
    }

# while (glob(WILDCARD))
    elsif (/\G \b while \s* \( \s* glob \b /oxgc) {
        return 'while ($_ = Ecyrillic::glob';
    }
...
# subroutines of package Ecyrillic
    elsif (/\G \b (CORE:: | ->[ ]* (?: atan2 | [a-z]{2,})) \b  /oxgc) { $slash = 'm//'; return $1;                  }
    elsif (/\G \b Cyrillic::eval       (?= \s* \{ )                /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Cyrillic::eval    \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'eval Cyrillic::escape'; }
    elsif (/\G \b bytes::substr \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'substr';            }
    elsif (/\G \b chop \b          (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Ecyrillic::chop';       }
    elsif (/\G \b bytes::index \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'index';             }
    elsif (/\G \b Cyrillic::index \b   (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Cyrillic::index';       }
    elsif (/\G \b index \b         (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Ecyrillic::index';      }
    elsif (/\G \b bytes::rindex \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'rindex';            }
    elsif (/\G \b Cyrillic::rindex \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Cyrillic::rindex';      }
    elsif (/\G \b rindex \b        (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Ecyrillic::rindex';     }
    elsif (/\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ecyrillic::lc';         }
    elsif (/\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ecyrillic::lcfirst';    }
    elsif (/\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ecyrillic::uc';         }
    elsif (/\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ecyrillic::ucfirst';    }
    elsif (/\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ecyrillic::fc';         }
    ...
    elsif (/\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ecyrillic::chr';               }
    elsif (/\G \b bytes::ord    (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ecyrillic::glob';              }
    elsif (/\G \b lc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Ecyrillic::lc_';               }
    elsif (/\G \b lcfirst \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Ecyrillic::lcfirst_';          }
    elsif (/\G \b uc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Ecyrillic::uc_';               }
    elsif (/\G \b ucfirst \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Ecyrillic::ucfirst_';          }
    elsif (/\G \b fc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Ecyrillic::fc_';               }
    ...
    elsif (/\G \b chr \b           (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Ecyrillic::chr_';              }
    elsif (/\G \b bytes::ord \b    (?! \s* => )                      /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord \b           (?! \s* => )                      /oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (/\G \b glob \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Ecyrillic::glob_';             }
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Ecyrillic::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Ecyrillic::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq       (\#) [ ] (\#) /oxgc) { return 'Ecyrillic::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq (\s*) (\() [ ] (\)) /oxgc) { return 'Ecyrillic::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { return 'Ecyrillic::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { return 'Ecyrillic::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { return 'Ecyrillic::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { return 'Ecyrillic::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q        (\#) [ ] (\#) /oxgc) { return 'Ecyrillic::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  (\s*) (\() [ ] (\)) /oxgc) { return 'Ecyrillic::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { return 'Ecyrillic::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { return 'Ecyrillic::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { return 'Ecyrillic::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { return 'Ecyrillic::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                ' [ ] '    /oxgc) { return 'Ecyrillic::split' . $e . qq     {' '};  }
        elsif (/\G                " [ ] "    /oxgc) { return 'Ecyrillic::split' . $e . qq     {" "};  }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> @{[Ecyrillic::PREMATCH()]}
            ...
            $e_string .= q{Ecyrillic::PREMATCH()};
            $slash = 'div';
        }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> @{[Ecyrillic::MATCH()]}
        elsif ($string =~ /\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
            $e_string .= q{Ecyrillic::MATCH()};
            $slash = 'div';
        }
...
# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> @{[Ecyrillic::POSTMATCH()]}
            ...
            $e_string .= q{Ecyrillic::POSTMATCH()};
            $slash = 'div';
        }
...
# subroutines of package Ecyrillic
        ...
        elsif ($string =~ /\G \b Cyrillic::eval       (?= \s* \{ )                /oxgc) { $e_string .= 'eval';              $slash = 'm//'; }
        elsif ($string =~ /\G \b Cyrillic::eval \b                                /oxgc) { $e_string .= 'eval Cyrillic::escape'; $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::substr \b                             /oxgc) { $e_string .= 'substr';            $slash = 'm//'; }
        elsif ($string =~ /\G \b chop \b                                      /oxgc) { $e_string .= 'Ecyrillic::chop';       $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::index \b                              /oxgc) { $e_string .= 'index';             $slash = 'm//'; }
        elsif ($string =~ /\G \b Cyrillic::index \b                               /oxgc) { $e_string .= 'Cyrillic::index';       $slash = 'm//'; }
        elsif ($string =~ /\G \b index \b                                     /oxgc) { $e_string .= 'Ecyrillic::index';      $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::rindex \b                             /oxgc) { $e_string .= 'rindex';            $slash = 'm//'; }
        elsif ($string =~ /\G \b Cyrillic::rindex \b                              /oxgc) { $e_string .= 'Cyrillic::rindex';      $slash = 'm//'; }
        elsif ($string =~ /\G \b rindex \b                                    /oxgc) { $e_string .= 'Ecyrillic::rindex';     $slash = 'm//'; }
        elsif ($string =~ /\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ecyrillic::lc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ecyrillic::lcfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ecyrillic::uc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ecyrillic::ucfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ecyrillic::fc';         $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ecyrillic::chr';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ecyrillic::glob';              $slash = 'm//'; }
        elsif ($string =~ /\G \b lc \b                                              /oxgc) { $e_string .= 'Ecyrillic::lc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst \b                                         /oxgc) { $e_string .= 'Ecyrillic::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b uc \b                                              /oxgc) { $e_string .= 'Ecyrillic::uc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst \b                                         /oxgc) { $e_string .= 'Ecyrillic::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b fc \b                                              /oxgc) { $e_string .= 'Ecyrillic::fc_';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr \b                                             /oxgc) { $e_string .= 'Ecyrillic::chr_';              $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::ord \b                                      /oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ /\G \b ord \b                                             /oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ /\G \b glob \b                                            /oxgc) { $e_string .= 'Ecyrillic::glob_';             $slash = 'm//'; }
...
# end of split
            if    ($string =~ /\G (?= [,;\)\}\]] )          /oxgc) { return 'Ecyrillic::split' . $e;                                           }

# split scalar value
            elsif ($string =~ /\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { $e_string .= 'Ecyrillic::split' . $e . e_string($1);  next E_STRING_LOOP; }

# split literal space
            elsif ($string =~ /\G \b qq       (\#) [ ] (\#) /oxgc) { $e_string .= 'Ecyrillic::split' . $e . qq  {qq$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Ecyrillic::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Ecyrillic::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Ecyrillic::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Ecyrillic::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Ecyrillic::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q        (\#) [ ] (\#) /oxgc) { $e_string .= 'Ecyrillic::split' . $e . qq   {q$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Ecyrillic::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Ecyrillic::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Ecyrillic::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Ecyrillic::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Ecyrillic::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G                ' [ ] '    /oxgc) { $e_string .= 'Ecyrillic::split' . $e . qq     {' '};  next E_STRING_LOOP; }
            elsif ($string =~ /\G                " [ ] "    /oxgc) { $e_string .= 'Ecyrillic::split' . $e . qq     {" "};  next E_STRING_LOOP; }
...
sub character_class {
    ...
    if ($char eq '.') {
        if ($modifier =~ /s/) {
            return '${Ecyrillic::dot_s}';
        }
        else {
            return '${Ecyrillic::dot}';
        }
    }
    else {
        return Ecyrillic::classic_character_class($char);
    }
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Ecyrillic::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Ecyrillic::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qq {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # octal escape sequence
            ...
            $char[$i] = Ecyrillic::octchr($1);
        }

        # hexadecimal escape sequence
        elsif ($char[$i] =~ /\A \\x \{ ([0-9A-Fa-f]+) \} \z/oxms) {
            $char[$i] = Ecyrillic::hexchr($1);
        }
        ...
        # \u \l \U \L \F \Q \E
            ...
            $char[$i] = '@{[Ecyrillic::ucfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\l') {
            $char[$i] = '@{[Ecyrillic::lcfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\U') {
            $char[$i] = '@{[Ecyrillic::uc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\L') {
            $char[$i] = '@{[Ecyrillic::lc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\F') {
            $char[$i] = '@{[Ecyrillic::fc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\Q') {
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Ecyrillic::charlist_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Ecyrillic::charlist_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # open character class [^...]
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    # [^...]
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Ecyrillic::charlist_not_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Ecyrillic::charlist_not_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ /\A [\x00-\xFF] \z/oxms) and (Ecyrillic::uc($char[$i]) ne Ecyrillic::fc($char[$i]))) {
            if (CORE::length(Ecyrillic::fc($char[$i])) == 1) {
                $char[$i] = '['   . Ecyrillic::uc($char[$i])       . Ecyrillic::fc($char[$i]) . ']';
            }
            else {
                $char[$i] = '(?:' . Ecyrillic::uc($char[$i]) . '|' . Ecyrillic::fc($char[$i]) . ')';
            }
        ...
        # $0 --> $0
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Ecyrillic::ignorecase(' . $char[$i] . ')]}';
            }
        }
        elsif ($char[$i] =~ /\A \$ \{ \s* 0 \s* \} \z/oxms) {
            if ($ignorecase) {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Ecyrillic::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Ecyrillic::ignorecase(Ecyrillic::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Ecyrillic::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Ecyrillic::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Ecyrillic::ignorecase(Ecyrillic::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Ecyrillic::MATCH()]}';
            }
        ...
        # $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Ecyrillic::POSTMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Ecyrillic::ignorecase(Ecyrillic::POSTMATCH())]}';
            }
            else {
                $char[$i] = '@{[Ecyrillic::POSTMATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $Cyrillic::re_t=%s; while($Cyrillic::re_t =~ %s){%s local $^W=0; local $Cyrillic::re_r=%s; %s$Cyrillic::re_t="$`$Cyrillic::re_r$'"; pos($Cyrillic::re_t)=length "$`$Cyrillic::re_r"; } return $Cyrillic::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Cyrillic::re_r=CORE::eval $Cyrillic::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Cyrillic::re_r=%s; %s"%s$Cyrillic::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $Cyrillic::re_n=0; while(%s =~ %s){%s local $^W=0; local $Cyrillic::re_r=%s; %s%s="$`$Cyrillic::re_r$'"; pos(%s)=length "$`$Cyrillic::re_r"; $Cyrillic::re_n++} return %s$Cyrillic::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Cyrillic::re_r=%s; %s%s="%s$Cyrillic::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Cyrillic::re_r=%s; %s%s="%s$Cyrillic::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Cyrillic::re_r=CORE::eval $Cyrillic::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
sub e_split {
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', 'Ecyrillic::split', $ope, $delimiter, @char, '>]}' x ($left_e - $right_e), $end_delimiter, $modifier;
    }
    return     join '', 'Ecyrillic::split', $ope, $delimiter, @char,                               $end_delimiter, $modifier;
}
...
This Ecyrillic module first appeared in ActivePerl Build 522 Built under
...
=item Split string
  ...
  @split = Ecyrillic::split(/pattern/,$string,$limit);
  @split = Ecyrillic::split(/pattern/,$string);
  @split = Ecyrillic::split(/pattern/);
  @split = Ecyrillic::split('',$string,$limit);
  @split = Ecyrillic::split('',$string);
  @split = Ecyrillic::split('');
  @split = Ecyrillic::split();
  @split = Ecyrillic::split;
  ...
  at all, Ecyrillic::split returns the original string as a single substring, If it
  ...
  @chars  = Ecyrillic::split(//,  $word);
  @fields = Ecyrillic::split(/:/, $line);
  @words  = Ecyrillic::split(" ", $paragraph);
  @lines  = Ecyrillic::split(/^/, $buffer);
  ...
  print join(":" => Ecyrillic::split(/ */, "hi there"));
  ...
  Ecyrillic::split's pattern is exempt from that wrinkle.)

  The $limit parameter splits only part of a string:

  my ($login, $passwd, $remainder) = Ecyrillic::split(/:/, $_, 3);
  ...
  splits on whitespace just as Ecyrillic::split with no arguments does. Thus,
  Ecyrillic::split(" ") can be used to emulate awk's default behavior. In contrast,
  Ecyrillic::split(/ /) will give you as many null initial fields as there are
  ...
  $string = join(" ", Ecyrillic::split(" ", $string));
  ...
  %head = ("FRONTSTUFF", Ecyrillic::split(/^(\S*?):\s*/m, $header));
  ...
  while (<>) {
      for my $word (Ecyrillic::split()) {
          $count{$word}++;
      }
  }

  The inverse of Ecyrillic::split is join, except that join can only join with the
...
=item Transliteration
  ...
  $tr = Ecyrillic::tr($variable,$bind_operator,$searchlist,$replacementlist,$modifier);
  $tr = Ecyrillic::tr($variable,$bind_operator,$searchlist,$replacementlist);
  ...
  print Ecyrillic::tr('bookkeeper','=~','boep','peob','r'); # prints 'peekkoobor'

=item Chop string

  $chop = Ecyrillic::chop(@list);
  $chop = Ecyrillic::chop();
  $chop = Ecyrillic::chop;

  This subroutine chops off the last character of a string variable and returns the
  character chopped. The Ecyrillic::chop subroutine is used primary to remove the newline
  ...
  since Ecyrillic::chop always shortens the string no matter what's there, and chomp
  is more selective. If no argument is given, the subroutine chops the $_ variable.

  You cannot Ecyrillic::chop a literal, only a variable. If you Ecyrillic::chop a list of
  variables, each string in the list is chopped:

  @lines = `cat myfile`;
  Ecyrillic::chop(@lines);

  You can Ecyrillic::chop anything that is an lvalue, including an assignment:

  Ecyrillic::chop($cwd = `pwd`);
  Ecyrillic::chop($answer = <STDIN>);

  This is different from:

  $answer = Ecyrillic::chop($tmp = <STDIN>); # WRONG

  which puts a newline into $answer because Ecyrillic::chop returns the character
  ...
  In the most general case, Ecyrillic::chop can be expressed using substr:

  $last_code = Ecyrillic::chop($var);
  $last_code = substr($var, -1, 1, ""); # same thing

  Once you understand this equivalence, you can use it to do bigger chops. To
  Ecyrillic::chop more than one character, use substr as an lvalue, assigning a null
  ...
  form of substr, creating something of a quintuple Ecyrillic::chop;
...
=item Index string
  ...
  $byte_pos = Ecyrillic::index($string,$substr,$byte_offset);
  $byte_pos = Ecyrillic::index($string,$substr);
  ...
  $byte_pos = -1;
  while (($byte_pos = Ecyrillic::index($string, $lookfor, $byte_pos)) > -1) {
...
=item Reverse index string
  ...
  $byte_pos = Ecyrillic::rindex($string,$substr,$byte_offset);
  $byte_pos = Ecyrillic::rindex($string,$substr);

  This subroutine works just like Ecyrillic::index except that it returns the byte
  position of the last occurrence of $substring in $string (a reverse Ecyrillic::index).
  ...
  while (($byte_pos = Cyrillic::rindex($string, $lookfor, $byte_pos)) >= 0) {
      print "Found at $byte_pos\n";
      $byte_pos--;
  }
...
=item Lower case string
  ...
  You can use the Ecyrillic::fc subroutine for case-insensitive comparisons via Cyrillic
  software.

=item Lower case first character of string

  $lcfirst = Ecyrillic::lcfirst($string);
  $lcfirst = Ecyrillic::lcfirst_;
...
=item Upper case string
  ...
  in interpolated strings. For titlecase, use Ecyrillic::ucfirst instead.
...
=item Upper case first character of string
  ...
  $ucfirst = Ecyrillic::ucfirst($string);
  $ucfirst = Ecyrillic::ucfirst_;
  ...
  $titlecase = Ecyrillic::ucfirst(substr($word,0,1)) . Ecyrillic::lc(substr($word,1));
  ...
  $do_not_use = Ecyrillic::ucfirst(Ecyrillic::lc($word));
...
=item Fold case string
  ...
  $fc = Ecyrillic::fc($string);
  $fc = Ecyrillic::fc_;

  New to Cyrillic software, this subroutine returns the full Unicode-like casefold of
  ...
  Ecyrillic::fc($a) eq Ecyrillic::fc($b)

  The reliable way to compare string case-insensitively was with the /i pattern
  modifier, because Cyrillic software has always used casefolding semantics for
...
=item Make ignore case string
  ...
  @ignorecase = Ecyrillic::ignorecase(@string);

  This subroutine is internal use to m/ /i, s/ / /i, split / /i, and qr/ /i.

=item Make capture number

  $capturenumber = Ecyrillic::capture($string);

  This subroutine is internal use to m/ /, s/ / /, split / /, and qr/ /.

=item Make character

  $chr = Ecyrillic::chr($code);
  $chr = Ecyrillic::chr_;

  This subroutine returns a programmer-visible character, character represented by
  that $code in the character set. For example, Ecyrillic::chr(65) is "A" in either
  ASCII or Cyrillic, not Unicode. For the reverse of Ecyrillic::chr, use Cyrillic::ord.

=item Filename expansion (globbing)

  @glob = Ecyrillic::glob($string);
  @glob = Ecyrillic::glob_;
  ...
  The Ecyrillic::glob subroutine grandfathers the use of whitespace to separate multiple
  ...
  @spacies = <"*e f*">;
  @spacies = Ecyrillic::glob('"*e f*"');
  @spacies = Ecyrillic::glob(q("*e f*"));

  If you had to get a variable through, you could do this:

  @spacies = Ecyrillic::glob("'*${var}e f*'");
  @spacies = Ecyrillic::glob(qq("*${var}e f*"));
