    ...
    for my $string (@string) {
        ...
        for (my $i=0; $i <= $#char; $i++) {
            ...
            # /i modifier
                ...
                my $lc = Char::Ecyrillic::lc($char[$i]);
                if ($uc ne $lc) {
                    $char[$i] = '[' . $uc . $lc . ']';
                }
            }
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Char::Ecyrillic::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Char::Ecyrillic::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  Scans a Cyrillic $string for delimiters that match pattern and splits the Cyrillic
  ...
  pattern, so the delimiters may be of any size and need not be the same Cyrillic
  $string on every match. If the pattern doesn't match at all, Char::Ecyrillic::split returns
  the original Cyrillic $string as a single substring. If it matches once, you get
  ...
  If Cyrillic $string is omitted, the function splits the $_ Cyrillic string.
...
=item Chop string
  ...
  Chops off the last character of a Cyrillic string contained in the variable (or
  Cyrillic strings in each element of a @list) and returns the character chopped.
  The Char::Ecyrillic::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Char::Ecyrillic::index($string,$substr,$position);
  $pos = Char::Ecyrillic::index($string,$substr);

  Returns the position of the first occurrence of $substr in Cyrillic $string.
  The start, if specified, specifies the $position to start looking in the Cyrillic
  $string. Positions are integer numbers based at 0. If the substring is not found,
  the Char::Ecyrillic::index function returns -1.

=item Reverse index string

  $pos = Char::Ecyrillic::rindex($string,$substr,$position);
  $pos = Char::Ecyrillic::rindex($string,$substr);

  Works just like Char::Ecyrillic::index except that it returns the position of the last
  occurence of $substr in Cyrillic $string (a reverse index). The function returns
  -1 if not found. $position, if specified, is the rightmost position that may be
  returned, i.e., how far in the Cyrillic string the function can search.
...
=item Lower case string
  ...
  Returns a lowercase version of Cyrillic string (or $_, if omitted). This is the
...
=item Lower case first character of string
  ...
  Returns a version of Cyrillic string (or $_, if omitted) with the first character
...
=item Upper case string
  ...
  Returns an uppercased version of Cyrillic string (or $_, if string is omitted).
...
=item Make character
  ...
  set. For example, Char::Ecyrillic::chr(65) is "A" in either ASCII or Cyrillic, not Unicode,
  and Char::Ecyrillic::chr(0x82a0) is a Cyrillic HIRAGANA LETTER A. For the reverse of
  Char::Ecyrillic::chr, use Char::Cyrillic::ord.
